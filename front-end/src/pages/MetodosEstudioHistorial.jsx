...
const nivelColor  = (p) => p >= 70 ? "#4caf7d" : p >= 40 ? "#e9a84c" : "#e05c5c";
const nivelLabel  = (p) => p >= 70 ? "Sólido" : p >= 40 ? "En desarrollo" : "Requiere atención";
const nivelCssKey = (p) => p >= 70 ? "solido" : p >= 40 ? "desarrollo" : "atencion";
...
export function MetodosEstudioHistorial() {
  ...
  useEffect(() => {
    api.get("/metodos-estudio/historial")
  ...
  return (
    <div className={`meh-app ${animado ? "meh-animated" : ""}`}>
      ...
      />
      <button className="meh-mute-btn" onClick={toggleMute} title={muted ? "Activar música" : "Silenciar"}>
      ...
      <div className="meh-header">
        <div className="meh-header-left">
          <button className="meh-back-btn" onClick={() => navigate("/metodos-estudio")}>← Volver</button>
        ...
        <div className="meh-header-right">
          ...
          <div className="meh-header-stat"><IoTimeOutline     size={14}/> Test CHTE</div>
          {mejorIntento && (
            <div className="meh-header-stat">
              <IoTrophyOutline size={14}/> Mejor: {Math.round(mejorIntento.puntaje_global || 0)}%
            </div>
          )}
        </div>
      </div>
      ...
      <div className="meh-main">
        ...
        {intentos.length > 0 && (
          <div className="meh-banner">
            <div className="meh-chip"><IoCalendarOutline size={14}/><span>{intentos.length} {intentos.length === 1 ? "intento" : "intentos"}</span></div>
            {mejorIntento && (
              <div className="meh-chip"><IoTrophyOutline size={14}/><span>Mejor: {Math.round(mejorIntento.puntaje_global || 0)}%</span></div>
            )}
            <div className="meh-chip"><IoTimeOutline size={14}/><span>Último: {formatFecha(intentos[0]?.fecha_intento)}</span></div>
          </div>
        )}
          ...
          <>
            <div className="meh-card">
              <div className="meh-card-body">
                <div className="meh-card-tag"><IoCalendarOutline size={11}/> Todos los intentos</div>
                  ...
                  <table className="meh-table">
                    ...
                    <tbody>
                      {intentos.map((intento, idx) => {
                        const puntaje       = Math.round(intento.puntaje_global || 0);
                        const esMasReciente = idx === 0;
                        return (
                          <tr
                            key={intento.id_intento}
                            className={`meh-table-row ${esMasReciente ? "meh-row--actual" : ""}`}
                            onClick={() => navigate("/resultado-metodos-estudio", { state: { id_intento: intento.id_intento } })}
                          >
                            ...
                            <td className="meh-col-puntaje">
                              <div className="meh-mini-bar-wrap">
                                <span className="meh-mini-val">{puntaje}%</span>
                                <div className="meh-mini-track">
                                  <div className="meh-mini-fill" style={{ width: `${puntaje}%`, background: nivelColor(puntaje) }}/>
                                </div>
                              </div>
                            </td>
                            <td className="meh-col-arrow"><IoArrowForwardOutline size={16}/></td>
                          </tr>
                        );
                      })}
                    </tbody>
